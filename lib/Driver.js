		...
		this.getCapabilities( device_data, ( err, capabilities ) => {
			...
			this._devices[ device_data.id ] = {
				...
				setInstanceProperty	: ( key, value ) => {
					...
					if( device.state.onoff !== true && ( instancePropertiesBlacklistWhenOff.indexOf(key) > -1 ) ) return;
				...
				setInstanceConfigProperty : ( key, value ) => {
					...
					if( device.state.onoff === false && ( key === 'on' ) ) return;
			...
			if( deviceInstance instanceof Error ) {
				if( deviceInstance.message === 'invalid_bridge'
				 ...
				 || deviceInstance.message === 'invalid_sensor' ) {
					...
					let onBridgeAvailable = function( bridgeId ){
																		
						let bridge = Homey.app.getBridge( bridgeId );
						...
						Homey.app.removeListener('bridge_available', onBridgeAvailable)
					...
					Homey.app.on('bridge_available', onBridgeAvailable);
				}
			} else {
			...
			.on('select_bridge', ( data, callback ) => {
				...
				for( let bridgeId in bridges ) {
					...
					if( typeof bridge.modelId === 'undefined' ) {
						this.debug('Skipping bridge due to undefined modelId', bridge);
						continue;
					}
